    ...
    public function get_table_data(Request $request) {
        ...
        return Datatables::eloquent($deposit_requests)
            ...
            ->addColumn('action', function ($deposit_request) {
                ...
                $actions .= '<a href="' . action('DepositRequestController@show', $deposit_request['id']) . '" class="btn btn-outline-primary btn-xs"><i class="ti-eye"></i> ' . _lang('Details') . '</a>&nbsp;';
                $actions .= $deposit_request->status != 2 ? '<a href="' . action('DepositRequestController@approve', $deposit_request['id']) . '" class="btn btn-outline-success btn-xs"><i class="ti-check-box"></i> ' . _lang('Approve') . '</a>&nbsp;' : '';
                $actions .= $deposit_request->status != 0 ? '<a href="' . action('DepositRequestController@reject', $deposit_request['id']) . '" class="btn btn-outline-warning btn-xs"><i class="ti-close"></i> ' . _lang('Reject') . '</a>&nbsp;' : '';
                $actions .= csrf_field();
