    ...
    public function get_table_data(Request $request) {
        ...
        return Datatables::eloquent($withdraw_requests)
            ...
            ->addColumn('action', function ($withdraw_request) {
                ...
                $actions .= '<a href="' . action('WithdrawRequestController@show', $withdraw_request['id']) . '" class="btn btn-outline-primary btn-xs"><i class="ti-eye"></i> ' . _lang('Details') . '</a>&nbsp;';
                $actions .= $withdraw_request->status != 2 ? '<a href="' . action('WithdrawRequestController@approve', $withdraw_request['id']) . '" class="btn btn-outline-success btn-xs"><i class="ti-check-box"></i> ' . _lang('Approve') . '</a>&nbsp;' : '';
                $actions .= $withdraw_request->status != 0 ? '<a href="' . action('WithdrawRequestController@reject', $withdraw_request['id']) . '" class="btn btn-outline-warning btn-xs"><i class="ti-close"></i> ' . _lang('Reject') . '</a>&nbsp;' : '';
                $actions .= csrf_field();
