(function ($) {
        ...
        .on("select2:open", () => {
          ...
          $(".select2-results:not(:has(a))").append(
            '<p class="border-top m-0 p-2"><a class="ajax-modal-2" href="' +
              ...
              '" data-title="' +
              $(this).data("title") +
              '" data-reload="false"><i class="ti-plus"></i> ' +
              $lang_add_new +
              "</a></p>"
          );
        });
          ...
          if (Array.isArray(json["message"])) {
            if (reload != false) {
              ...
              jQuery.each(json["message"], function (i, val) {
                $("#main_modal .alert-danger").append(
                  "<span>" + val + "</span>"
                );
              });
              $("#main_modal .alert-primary").addClass("d-none");
  ...
  $("#user_type").val() == "user"
    ? $("#user_branch_id").prop("disabled", false)
    : $("#user_branch_id").prop("disabled", true);
  ...
  if ($(".datepicker").length) {
    $(".datepicker").each(function (i, obj) {
      ...
      if (typeof $(this).next().attr("class") === "undefined") {
        ...
        $(this).next(".fake_datepicker").css("margin-top", "-47.2px");
      }
      $(this)
...
function showRole(elem) {
  ...
  window.location = _admin_url + "/permission/access_control/" + $(elem).val();
}
