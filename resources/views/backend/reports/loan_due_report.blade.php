					...
					@if(isset($report_data))
						@foreach($report_data as $repayment)
							<tr>
								...
								<td><a href="{{ action('MemberController@show', $repayment->loan->borrower_id) }}" target="_blank">{{ $repayment->loan->borrower->member_no }}</a></td>
								<td>{{ $repayment->loan->borrower->name }}</td>
								<td>{{ $repayment->loan->borrower->mobile }}</td>
